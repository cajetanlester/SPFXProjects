...
import * as strings from 'DynamicAccordionWebPartStrings';
import DynamicAccordion from './components/DynamicAccordion';
import { IDynamicAccordionProps } from './components/IDynamicAccordionProps';

export interface IDynamicAccordionWebPartProps {
  listId: string;
...
export default class DynamicAccordionWebPart extends BaseClientSideWebPart<IDynamicAccordionWebPartProps> {
  ...
  public render(): void {
    const element: React.ReactElement<IDynamicAccordionProps> = React.createElement(
      DynamicAccordion,
      {
    ...
    return new Promise<IPropertyPaneDropdownOption[]>(
      (
        ...
        const spListColumns = this._sp.web.lists
          ...
          ();
        spListColumns.then((columnResult: any[]) => {
          const listColumns: { key: any; text: any; }[] = [];
          columnResult.forEach((column: { Title: any; }) => {
            listColumns.push({
              key: column.Title,
              text: column.Title,
            });
    ...
    return new Promise<IPropertyPaneDropdownOption[]>(
      (
        ...
        const spListColumns = this._sp.web.lists
          ...
          ();
        spListColumns.then((columnResult: any[]) => {
          const listColumns: { key: any; text: string; }[] = [];
          columnResult.forEach((column: { InternalName: string; Title: string; }) => {
            listColumns.push({
    ...
    return new Promise<IPropertyPaneDropdownOption[]>(
      (
        ...
        choices.then((result) => {
          ...
          const columnChoices: { key: any; text: any; }[] = [];
